...
module.exports = {
	...
	list: async () => {
		const results = await axios.get('http://localhost:8004/Planet');
		return results.data;
	},

	getById: async (id) => {
		const results = await axios.get(`http://localhost:8004/Planet/${id}`);
		return results.data;
	},

	insert: async (planet) => {
		const results = await axios.post('http://localhost:8004/Planet', planet);
		return results.data;
	},

	delete: async (id) => {
		const results = await axios.delete(`http://localhost:8004/Planet/${id}`);
		return results.data;
	},
};
